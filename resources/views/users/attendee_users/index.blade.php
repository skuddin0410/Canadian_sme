						...
						<div class="dt-buttons"> 
                             <a href="{{route('attendee-users.create')}}" class="dt-button create-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button">
								<span><i class="bx bx-plus me-sm-1"></i> 
									<span class="d-none d-sm-inline-block">Add Attendee</span>
								</span>
							</a> 
...
<script type="text/javascript">
 function GetUserList() {
   ...
   $(document).on("click", ".filter", function(e) {
       ...
       $(".spinner-border").fadeIn(300);  
       $.ajax({
            url: "{{route('attendee-users.index')}}" + '?' + $("#users-filter").serialize(),
            type: 'GET',
            headers: {
                'X-CSRF-Token': $('meta[name="_token"]').attr('content')
            },
    ...
    $('.reset-filter').on('click', function() {
      window.location.href = "{{route('attendee-users.index')}}";
    });   	
</script>	
@endsection
