...
<tbody>	
    ...
    <tr>
		...
		{{-- <th>
			<div class="row">
			<div class="col-4 p-1">	
				<a href="{{ route('representative-users.show', $user->id) }}" class="btn btn-sm btn-icon item-show"><i class="bx bxs-show"></i></a>
            </div>
		    <div class="col-4 p-1">	
			<a href="{{ route('representative-users.edit', $user->id) }}" class="btn btn-sm btn-icon item-edit"><i class="bx bxs-edit"></i></a>
            </div>
		<div class="col-4 p-1">
			<form action="{{ route('representative-users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this representative user?');">
            @csrf
            @method('DELETE')
  ...
  <div class="d-flex gap-2">
    {{-- View --}}
    <a href="{{ route('representative-users.show', $user->id) }}" class="btn btn-sm btn-icon btn-primary" title="View">
      <i class="bx bx-show"></i>
    </a>

    {{-- Edit --}}
    <a href="{{ route('representative-users.edit', $user->id) }}" class="btn btn-sm btn-icon item-edit" title="Edit">
