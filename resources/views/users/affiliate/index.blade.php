...
<div class="container flex-grow-1 container-p-y pt-0">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light"> Affiliate Manager/</span>Lists</h4>
    <div class="row">
        <div class="col-xl">
            <div class="card mb-4">
				<div class="card-header d-flex justify-content-between align-items-center">
				    <h5 class="mb-0"> Affiliate Manager List</h5>
					<div class="dt-action-buttons text-end pt-3 pt-md-0">
						<div class="dt-buttons"> 
							<a href="{{route('affiliates.create')}}" class="dt-button create-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button">
								<span><i class="bx bx-plus me-sm-1"></i> 
									<span class="d-none d-sm-inline-block">Add Affiliate Manager</span>
								</span>
							</a> 
						</div>
					</div>
...
<script type="text/javascript">
 function GetUserList() {
   ...
   $(document).on("click", ".filter", function(e) {
        ...
        var kyc = $('#kyc').val();
        if( search.trim() == '' && kyc.trim()== ''){
           return ;
        }
       $(".spinner-border").fadeIn(300);  
       $.ajax({
            url: "{{route('affiliates.index')}}" + '?' + $("#users-filter").serialize(),
            type: 'GET',
            headers: {
                'X-CSRF-Token': $('meta[name="_token"]').attr('content')
            },
    ...
    $('.reset-filter').on('click', function() {
      window.location.href = "{{route('affiliates.index')}}";
    });   	
</script>	
@endsection
