...
<tbody>	
    ...
    <tr>
		...
		<th>{{$user->mobile ?? ''}}</th>
		<th>{{$user->referral_coupon}}</th>
		<th>{{$user->referral_percentage}} %</th>
		<th>{{dateFormat($user->created_at) ?? '' }}</th>
		<th>
			<div class="row">
			<div class="col-4 p-1">	
				<a href="{{ route("affiliates.show",["affiliate"=> $user->id ]) }}" class="btn btn-sm btn-icon item-show"><i class="bx bxs-show"></i></a>
            </div>
		    <div class="col-4 p-1">	
			<a href="{{ route("affiliates.edit",["affiliate"=> $user->id ]) }}" class="btn btn-sm btn-icon item-edit"><i class="bx bxs-edit"></i></a>
            </div>
       </th>
	</tr>
	@endforeach
