...
<div class="container-xxl flex-grow-1 container-p-y pt-0">
  <h4 class="py-3 mb-4"><span class="text-muted fw-light"> Affiliate Manager/</span>@if(!empty($user)) Update @else Create @endif</h4>
  <div class="row">
    <div class="col-xl">
      <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="mb-0"> Affiliate Manager @if(!empty($user)) Update @else Create @endif</h5>
        </div>
        <div class="card-body">
          ...
          @endif
            ...
            <form  action="@if(!empty($user)) {{ route('affiliates.update',['affiliate'=>$user->id]) }} @else {{ route('affiliates.store') }} @endif " method="POST" autocomplete="off" enctype="multipart/form-data" >
            {{ csrf_field() }}
             @if(!empty($user))
             @method('PUT')
            @endif
          ...
          <div class="col-6">
           ...
           <div class="col-3">
            <div class="mb-3">
                  <label class="form-label" for="title">Referral percentage<span class="text-danger">* (1 to 100 allowed)</span></label>
                  <div class="input-group input-group-merge">
                    <span id="title-icon" class="input-group-text">%</span>
                    <input
                      type="text"
                      class="form-control"
                      name="referral_percentage"
                      id="referral_percentage"
                      value="{{ $user->referral_percentage ?? old('referral_percentage') }}"
                      placeholder="User referral coupon"
                      {{!empty($user) && $user->referral_percentage ? ' readonly':'' }}
                     
                      />
                  </div>
                  @if ($errors->has('referral_percentage'))
                    <span class="text-danger text-left">{{ $errors->first('referral_percentage') }}</span>
                  @endif
            </div>
           </div>
           <div class="col-6">
          ...
          <div class="col-12">
            <div class="mb-3">
              <div class="d-flex pt-3 justify-content-end">
                <a href="{{route("affiliates.index")}}" class="btn btn-outline-primary btn-pill btn-streach font-book ml-3 mt-6 fs-14 me-2">Cancel</a>
