...
<div class="container-xxl flex-grow-1 container-p-y pt-0">
                        ...
                        <ul class="list-unstyled justify-content-between">  
                          ...
                          <li class="mb-3">
                            ...
                            <span>{{ $user->referral_coupon?? '' }}</span>
                              ...
                              {{-- <li class="mb-3">
        <span class="fw-medium me-2">Approval Status:</span>
        @if ($user->is_approve)
    ...
    {{-- <li class="mb-3">
    <span class="fw-medium me-2">Approval Status:</span>
    <span>Raw value: {{ $user->is_approve }}</span> 
    @if ($user->is_approve)
        <span class="badge badge-success">Approved</span>
    @else
        <span class="badge badge-warning">Pending</span>
...
<li class="mb-3">
    <span class="fw-medium me-2">Approval Status:</span>
    <span>Raw: {{ $user->is_approve }}</span><br> {{-- Debugging --}}
    @if ($user->is_approve == 1 || $user->is_approve === true)
        <span class="badge badge-success">Approved</span>
    @else
    ...
    <div class="d-flex pt-3 justify-content-end">
    @if (!$user->is_approve)
        <form action="{{ route('exhibitor-users.approve', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to approve this user?');">
            @csrf
            @method('PATCH')
    ...
    @endif
        ...
        {{-- <button type="button" class="btn btn-outline-secondary me-2" disabled>Approved</button>
    @endif
    <a href="{{ route('exhibitor-users.index') }}" class="btn btn-outline-primary">Back</a> --}}
