        ...
        <div class="col-xl">
            <div class="card mb-4">
				<div class="card-header d-flex justify-content-between align-items-center">
				    <h5 class="mb-0">Banner Lists</h5>
					<div class="dt-action-buttons text-end pt-3 pt-md-0">
						<div class="dt-buttons"> 
                            @if(Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Event Admin'))
							<a href="{{route('banners.create')}}" class="dt-button create-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button">
								<span><i class="bx bx-plus me-sm-1"></i> 
									<span class="d-none d-sm-inline-block">Add Banner</span>
								</span>
							</a> 
                            @endif
						</div>
...
<script type="text/javascript">
	...
	$(document).on("focusout", ".editOrder", function(e) {
      ...
      var order = $('#'+id).text();
		...
		let link = '/banners/'+id+'/order/'+ order
		$.ajax({
		    url: link,
		}).done(function(data) {
		    GetBannerList();
 ...
 function GetBannerList() {
        $.ajax({
            ...
            success: function (data) {
               $(document).find("#banner-table").html(data.html);
            },
            error:function(data){
