        ...
        <div class="col-xl">
            <div class="card mb-4">
				<div class="card-header d-flex justify-content-between align-items-center">
				    <h5 class="mb-0">Category  Or Tag Lists</h5>
					<div class="dt-action-buttons text-end pt-3 pt-md-0">
						<div class="dt-buttons"> 
							<a href="{{route('categories.create')}}" class="dt-button create-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button">
								<span><i class="bx bx-plus me-sm-1"></i> 
									<span class="d-none d-sm-inline-block">Add Category Or Tag</span>
								</span>
							</a> 
                ...
                <form action="#" class="" method="GET" id="category-filter">        
                    <div class="row">
                        <div class="col-4"></div>
                        <div class="col-3"></div>          
...
<script type="text/javascript">
	...
	$(document).on("focusout", ".editOrder", function(e) {
      ...
      var order = $('#'+id).text();
        order = order.trim();
        if(isNumeric(order) && order >= 0){
		let link = '/categories/'+id+'/order/'+ order
		$.ajax({
		    url: link,
		}).done(function(data) {
		    GetCategoryList()
...
<script type="text/javascript">
	 function GetCategoryList() {
        ...
        $.ajax({
            ...
            success: function (data) {
               $(document).find("#category-table").html(data.html);
  ...
  $(document).on("click", ".filter", function(e) {
       ...
       $(".spinner-border").fadeIn(300);
       $.ajax({
            url: "{{route('categories.index')}}" + '?' + $("#category-filter").serialize(),
            type: 'GET',
            headers: {
                'X-CSRF-Token': $('meta[name="_token"]').attr('content')
            },
