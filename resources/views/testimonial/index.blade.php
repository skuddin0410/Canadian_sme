				...
				<div class="card-header d-flex justify-content-between align-items-center">
					...
					<div class="dt-action-buttons text-end pt-3 pt-md-0">
						...
						<div class="dt-buttons"> 
							<a href="{{route('testimonials.create')}}" class="dt-button create-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button">
								<span><i class="bx bx-plus me-sm-1"></i> 
									<span class="d-none d-sm-inline-block">Add Testimonial</span>
								</span>
							</a> 
						</div>
                       @endif 
...
<script type="text/javascript">
	...
	$(document).on("focusout", ".editOrder", function(e) {
      ...
      var order = $('#'+id).text();
		...
		let link = '/testimonials/'+id+'/order/'+ order
		$.ajax({
		    url: link,
		}).done(function(data) {
		    GetTestimonialList()
 ...
 function GetTestimonialList() {
        $.ajax({
            ...
            success: function (data) {
               $(document).find("#testimonial-table").html(data.html);
            },
            error:function(data){
